...
class BFSCache{
	...
	std::unordered_map<const State*, std::unordered_set<const State*, std::hash<State*>, std::equal_to<State*>>, std::hash<State*>, std::equal_to<State*>> adj;
	int size() const { return setTT.size(); };
	int getExits() const ;
	const State* getFirst() const { return *setTT.begin();};
	void setSolvable();
	bool isSolvable() const;
	double getLocalCCSum() const;
	int getDiameter() const;
	int getRadius() const;
	double getBranchingFactor() const;
	double getFiedlerValue() const;
	int getMaxSolutionLength() const;
	...
	const State* getCanonical(const State* s) const {
		auto it = setTT.find(s);
		if(it != setTT.end()) return *it;
		return nullptr;
	}
